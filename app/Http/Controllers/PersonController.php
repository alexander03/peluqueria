...
class PersonController extends Controller
{
    public function employeesautocompleting($searching)
    {
        ...
        $resultado = Persona::where(function($query){    
                                $query->where('type', '=', 'E');
    ...
    public function providersautocompleting($searching)
    {
        $entidad   = 'Person';
        $mdlPerson = new Persona();
        $resultado = Persona::where('type', '=', 'P')->where('bussinesname', 'LIKE', '%'.$searching.'%');
        $lista     = $resultado->get();
        $data      = array();
    ...
    {
        $entidad    = 'Person';
        $mdlPersona = new Persona();
        $resultado  = Persona::where('type', '=', 'C')->where('bussinesname', 'LIKE', '%'.strtoupper($searching).'%')->orWhere(DB::raw("CONCAT(lastname,' ',firstname)"),'LIKE' ,'%'.strtoupper($searching).'%');
        $lista      = $resultado->get();
        $data       = array();
        foreach ($lista as $key => $value) {
            $customername = "";
